...
**추천 기준** — LLM 을 쓰지 않는 조회 조건이다.

뽑는 대상

| 조건 | 이유 |
|---|---|
| 동반 가능(`status=allowed`) | 못 데려가는 곳을 홈에 띄울 수 없다 |
| 정책 확인 완료(`needs_verification=false`) | "동반 정책 확인 필요" 라벨이 붙은 곳은 대표 자리에 두지 않는다 |
| 대표 사진 있음 | 가로 스크롤 카드가 사진 없이는 빈칸으로 보인다 |
| 카테고리 화이트리스트 | `attraction` `cafe` `restaurant` `culture` `leisure` `park` `beach` 만. 숙소·쇼핑은 홈탭에서 찾는 것이 아니고(쇼핑은 수집 데이터의 절반이 아울렛 매장이다), 동물병원·애견미용실은 여행지가 아니다 |

순서

1. **카테고리 라운드로빈** — 관광지1 · 맛집1 · 카페1 · 관광지2 … 그냥 줄세우면 10칸이 전부 오름으로 찬다
2. 같은 카테고리 안에서는 **동반 정책 신뢰도** 순 (관광공사 0.90 > 비짓제주 태그 0.75 > 소개글 마이닝 0.70)
3. 그 다음 사진 수 → `id`

> **"인기"는 아직 랭킹에 넣지 않았다.** 저장수·평점이 0건이다. 비짓제주 원문에 조회수(`readcnt`)가 들어 있으니 그걸 쓰려면 컬럼을 하나 늘려야 한다.
