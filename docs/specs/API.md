...
>
> **등록 화면은 2단계다** — step1 이름·종·품종 / step2 크기·성향.
> **위 다섯 개가 등록에 쓰는 전부다.** 성별·중성화·사진·몸무게는 받지 않는다.
>
> **`size` 는 몸무게 구간으로 고른다.** 화면 선택지가 이 셋이다.
>
> | 화면 | 보낼 값 |
> |------|---------|
> | 10kg 미만 | `small` |
> | 10kg 이상 ~ 25kg 이하 | `medium` |
> | 25kg 초과 | `large` |
>
> 서버는 구간을 다시 검사하지 않는다. 화면에서 고른 값을 그대로 저장한다.
>
> **`traits` 는 5개 중 최대 3개.** 화면 문구와 보낼 값의 대응이다.
>
> | 화면 | 보낼 값 |
> |------|---------|
> | 활발해요 | `active` |
...
> | 호기심이 많아요 | `curious` |
>
> 0개도 허용한다. 중복이나 목록 밖의 값은 `invalid_traits` 로 거절한다.
...
| 필드 | 타입 | 설명 |
|------|------|------|
| `name`·`species`·`breedId`·`size`·`traits` | | 보낸 것만 갱신 |
...
**반려동물을 반영하려면** `?petIds=` 를 붙인다. 값이 없으면 개인화하지 않는다.

```
GET /api/places/recommended                     비개인화 (홈탭)
GET /api/places/recommended?petIds=me           등록한 반려동물 전체
GET /api/places/recommended?petIds=<id>,<id>    고른 반려동물만
```

**여러 마리면 전부 들어갈 수 있어야 하므로 가장 큰 개가 기준이 된다.** 말티즈와
리트리버를 함께 데려가면 대형견 기준으로 본다.

**못 들어가는 곳도 목록에는 남고 순위만 내려간다.** 크기·무게 제한이 적힌 장소가
전체의 13% 뿐이라 걸러내면 갈 수 있는 곳까지 잃는다. `petConditions` 에
`6kg 이하` 같은 문구가 이미 나가므로 화면에서 판단할 수 있다.

크기는 구간으로만 받으므로 구간 안에서 가장 무거울 수 있는 값으로 판정한다.

| 등록한 크기 | 통과하는 장소 |
|---|---|
| 소형 (10kg 미만) | 무게 제한이 없거나 10kg 이상 |
| 중형 (10~25kg) | 무게 제한이 없거나 25kg 이상 |
| 대형 (25kg 초과) | 무게 제한이 없는 곳만 |

`petIds` 에 남의 반려동물을 넣으면 `invalid_pets` (422) 다.
